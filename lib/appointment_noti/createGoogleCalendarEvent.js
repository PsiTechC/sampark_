...
export async function createGoogleCalendarEvent(accessToken, eventDetails, refreshToken, userId, db, tokenKey = "") {
  ...
  if (res.status === 401 && refreshToken) {
    ...
    if (tokenKey !== undefined && tokenKey !== null) {
      const tokenField = tokenKey === 0
      ? "googleAccessToken"
      : typeof tokenKey === "number"
        ? `googleAccessToken${tokenKey}`
        : "googleAccessToken";

        console.log(`🔄 Refreshing token for tokenKey:`, tokenKey);
        console.log(`💾 Will save refreshed token to field: ${tokenField}`);
