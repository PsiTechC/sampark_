...
export default function useBatchWSPolling() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      fetch(`${BASE_URL}/api/clients/forWSBolna`)
        .then((res) => {
          if (!res.ok) {
            throw new Error("Failed to fetch forWSBolna");
          }
          return res.json();
        })
        .then((data) => {
          console.log("✅ forWSBolna completed", data);
          return fetch(`${BASE_URL}/api/clients/sendWSBolna`);
        })
        .then((res) => {
          if (!res.ok) {
            throw new Error("Failed to fetch sendWSBolna");
          }
          return res.json();
        })
        .then((sendData) => {
