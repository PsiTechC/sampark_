...
//       if (isLoginMode) {
//         const redirectUrl = `/client-dashboard/${user._id}`; // Construct the dynamic URL using the user._id
//         router.push(redirectUrl); // Redirect to the client-specific dashboard
//       }
...
//       if (isLoginMode) {
//         const { user, token, redirectUrl } = response.data; // Extract user, token, and redirectUrl from the response
//         localStorage.setItem('token', token); // Store JWT token in localStorage for future requests
//         router.push(redirectUrl); // Redirect to the client-specific dashboard
//       } else {
//         setIsOtpMode(true); // Switch to OTP mode after successful registration
//         setSuccessMessage('Registration successful! Check your email for the OTP to verify your account.');
...
// //C:\botGIT\botGIT-main\pages\register.js
...
//         router.push(`/verify-email?userId=${userId}`); // Redirect to verify-email page
//       }
//     } catch (error) {
//       console.error('Error during registration/login:', error);
//       setError(error.response?.data?.message || 'Something went wrong');
...
//     try {
//       const response = await axios.post('/api/register/verify-otp', { email, otp });
//       setSuccessMessage('Email verified successfully! Redirecting to login...');
//       setTimeout(() => {
//         router.push('/login');
//       }, 2000);
...
//         <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
//           {isLoginMode ? (
//             <LoginComponent /> // Use LoginComponent if in login mode
//           ) : (
//             <>
//               <h1 className="text-2xl font-semibold mb-6 text-center">{isOtpMode ? 'Verify OTP' : 'Register'}</h1>
//               {successMessage && <p className="text-green-500 mb-4">{successMessage}</p>}
//               {error && <p className="text-red-500 mb-4">{error}</p>}
